...
> sayso は個人プロジェクトです。Anthropic 公式の製品・モデルではありません（"Sonnet" は Anthropic のモデル名です）。
...
Anthropic のモデルは Mythos（神話）、Fable（寓話）と、物語の格を降りながら名付けられている。このフレームワークはその遥か下を名乗る。**sayso**（say-so）は「人の言い分」「又聞き」——もはや物語ですらない、伝聞である。

実際、これは Fable 本人ではない。Fable の実セッションの振る舞いという「証言」から、行動規範を又聞きで再構成したもの——文字通り、誰かの say-so で動いている。（旧称: neosonnet）
...
| `eval/protocol.md` | 評価プロトコル。ライブタスク比較・ブラインド多重ジャッジ・ベンチ最適化の防止規律 |
...
**sayso は「フレームワーク＋Sonnet 5」を一つの仮想モデルとして扱う名前**。前提条件: [Claude Code](https://claude.com/claude-code) CLI がインストール・ログイン済みであること。推奨の起動形態はシステムプロンプト注入で、`sayso` コマンドとして立ち上げる:

```sh
git clone https://github.com/maguroid/sayso.git ~/.sayso && ~/.sayso/install.sh
```

インストール後は `sayso` で起動する。更新は `sayso update`、インストール済みバージョンの確認は `sayso version`。

- `~/.sayso` にフレームワークを設置し、`~/.local/bin/sayso` から起動する
- `SAYSO_HOME` と `SAYSO_MODEL` で設置先・モデル名を上書きできる
...
- 代替: zsh の alias / 関数で `claude --model claude-sonnet-5 --append-system-prompt "$(cat ~/ghq/github.com/maguroid/sayso/framework/ORCHESTRATOR.md)"` を定義してもよい
